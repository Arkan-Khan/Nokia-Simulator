...
// Fit behavior: 'contain' (default) scales to fit viewport; 'exact' uses 1:1 canvas pixels (no zoom)
const fitMode = (sp.get('fit') || 'contain').toLowerCase();
...
// Optional forced resolution from URL params (e.g., ?w=240&h=320)
const forcedWidth = sp.get('w') ? parseInt(sp.get('w'), 10) : null;
const forcedHeight = sp.get('h') ? parseInt(sp.get('h'), 10) : null;
let scaleSet = false;
...
function autoscale() {
    ...
    if (fitMode === 'exact') {
        // No zooming; show 1:1 canvas, centered by #screen-area flex
    ...
    await cheerpjInit({
        ...
        natives: {
            ...
            async Java_pl_zb3_freej2me_bridge_shell_Shell_setCanvasSize(lib, width, height) {
                // If width/height are forced via URL, override
                if (Number.isInteger(forcedWidth) && Number.isInteger(forcedHeight) && forcedWidth > 0 && forcedHeight > 0) {
                    width = forcedWidth;
                    height = forcedHeight;
                }
